$(document).ready(function() {
	$('input').click(function() {
		if($('#creditcard').is(':checked')){
			$('#enter-form').empty();
			$("#enter-form").append('<label class="ccjs-num">Card number<input name="card-number" placeholder="**** **** **** ****" id="id-card-number"><label>');	
			...
			//check if creditcard only has numeric values
            var crdNum = $('#id-card-number').val();
            $('#id-card-number').keyup(function() {
                if(GetCardType(crdNum) == "Visa"){
                    $("#typeCard").text("Visa");
                }
            });
		} else if($('#baypal').is(':checked')) {
			$('#enter-form').empty();
			$('#enter-form').append('<label class="baypal-email">Email<input name="baypal-email input-baypal-email"></label>');
			$('#enter-form').append('<label class="baypal-password">Password<input name="baypal-password input-baypal-password"></label>');
		}
	});
